 ...
 * <p>
 * 1. headers - the key for this key-value pair is "headers". The value for this
 * key is another map, which represent the event headers. These headers are
 * inserted into the Flume event as is.
 * <p>
 * 2. body - The body is a string which represents the body of the event. The
 * key for this key-value pair is "body". All key-value pairs are considered to
 * be headers. An example:
 * <p>
 * [{"headers" : {"a":"b", "c":"d"},"body": "random_body"}, {"headers" : {"e":
 * "f"},"body": "random_body2"}]
 * <p>
 * would be interpreted as the following two flume events:
 * <p>
 * * Event with body: "random_body" (in UTF-8/UTF-16/UTF-32 encoded bytes) and
 * headers : (a:b, c:d)
 * <p>
 ...
 * required).
 * 
 * One way to create an event in the format expected by this handler, is to use
 * {@linkplain JSONEvent} and use {@linkplain Gson} to create the JSON string
 * using the {@linkplain Gson#toJson(java.lang.Object, java.lang.reflect.Type) }
 * method. The type token to pass as the 2nd argument of this method for list of
...
public class JSONHandler implements NettyHTTPSourceHandler {

	private static final Logger LOG = LoggerFactory.getLogger(JSONHandler.class);
	private final Type listType = new TypeToken<List<JSONEvent>>() {
	}.getType();
	private final Type eventType = new TypeToken<JSONEvent>() {
	}.getType();
	...
	public List<Event> getEvents(String request) throws Exception {
		/*
		...
		try {
			eventList = gson.fromJson(request, listType);
		} catch (JsonSyntaxException ex) {
			try {
				Event event = gson.fromJson(request, eventType);
				eventList.add(event);
			} catch (JsonSyntaxException ex2) {
				throw new NettyHTTPBadRequestException("Request has invalid JSON Syntax.", ex2);
			}
		}
